...
//NO se puede usar map por que devuelve menos elementos que del tamaño de la lista

const getDurationMovies = (dataArray,minute1,minute2) =>{
   
    let movie  = dataArray.map((item =>{
        let duration = parseInt(item.duration)
        if (duration > minute1 && duration < minute2) {
            return [duration]
        }
    }))
...
console.log(getDurationMovies(movies,130,180))

//Este si funciona y nos filtra todo :D!
const getDurationMovies1 = (dataArray,minute1,minute2) =>{
...
console.log(getDurationMovies1(movies,130,180))