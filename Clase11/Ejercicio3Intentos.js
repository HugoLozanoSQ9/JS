...
const getFirstActor = (dataArray) =>{ //Ex.3
  let actorName = dataArray.reduce((accum,current) =>{
      return accum.includes(current.protagonists[0].name) ? accum : [...accum, current.protagonists[0].name]
      
  },[])
  return actorName
}

const getSecontActor = (dataArray) =>{ //Ex.3
  let actorName = dataArray.reduce((accum,current) =>{
      return accum.includes(current.protagonists[1].name) ? accum : [...accum, current.protagonists[1].name]
      
  },[])
  return actorName
}

const getActorNames = (listFirst,listSecond) =>{ //Ex.3
  let finalList = listFirst.concat(listSecond)
  return finalList
}

console.log(getActorNames(getFirstActor(movies), getSecontActor(movies)))
