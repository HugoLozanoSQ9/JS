const movies = [
  ...
  /*
    ...
    */
    ...
    const getGenero = (dataArray) =>{ //Ex1
        let result = dataArray.reduce ((accum,current) =>{
        return accum.includes(current.genre) ? accum : [...accum, current.genre]
        },[])
        return result
    }
    ...
    //console.log(getDirectorsName(movies)) //Ex2

    const getActorNames = (dataArray) =>{ //Ex.3
        let actorName = dataArray.reduce((accum,current) =>{
            
            return accum.includes(current.protagonists[0].name) || accum.includes(current.protagonists[1].name) ? accum : [...accum, current.protagonists[0].name,current.protagonists[1].name,]
            
        },[])
        return actorName
    
    }    
    //console.log(getActorNames(movies)) //Ex3

    const getRatings = (dataArray) =>{ //Ex4
        let rating = dataArray.reduce((accum,current) =>{
            return accum.includes(current.rating) ? accum:[...accum,current.rating]
        },[])
        return rating
    }
    //console.log(getRatings(movies)) //Ex4

    const getDurationMovies = (dataArray,minute1,minute2) =>{ //Ex.5
         let movie = dataArray.reduce((accum,current) =>{
             let duration = parseInt(current.duration)
             return (duration >= minute1 && duration <= minute2) ? [...accum,current.title] : accum 
         },[])
         return movie
     }
    //console.log(getDurationMovies(movies,130,180)) //Ex. 5

    const getRatingsAndMovies = (dataArray) =>{ //Ex6
        let rating = dataArray.reduce((accum,current) =>{
            return accum.includes(current.title) ? accum:[...accum,[current.rating, current.title]]
        },[])
    
        let moviesRated = rating.map((item)=>{
            return{
                name: item[1],
                rate: item[0]
            }
    ...
    //console.log(getRatingsAndMovies(movies)) //Ex6
    

    const getYearsMovies = (dataArray,year1,year2) =>{ //Ex.7
        let movie = dataArray.reduce((accum,current) =>{
            let relaseYear = parseInt(current.releaseYear)
            return (relaseYear >= year1 && relaseYear <= year2) ? [...accum,current.title] : accum 
        },[])
        return movie
    }
    //console.log(getYearsMovies(movies,1993,1994)) //Ex. 7
    
    const getMoviesFromContry = (dataArray) =>{ //Ex8
        let country = dataArray.reduce((accum,current) =>{
            return accum.includes(current.country) ? accum:[...accum,[current.country, current.title]]
        },[])
    
        let moviesByCountry = country.map((item)=>{
            return{
                name: item[1],
                country: item[0]
            }
    ...
    //console.log(getMoviesFromContry(movies)) //Ex8

    const noOscarMovies = (dataArray) =>{  //Ex9
        let oscar= dataArray.filter((item)=>{
            return item.oscarAwards == 0
        })
        return oscar        
    }
    //console.log(noOscarMovies(movies)) //Ex9

    const getObjFormat = (dataArray) => { //Ex10
      let solution = {
        'PG-13': 0,
        'R': 0,
        'PG': 0,
    ...
    //console.log( getObjFormat(getRatingsAndMovies(movies))) //Ex10
